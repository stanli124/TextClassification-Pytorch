...
class RCNN(Model):
    def __init__(self,
        ...
        self.forward_rnn = SimpleRNN(128, return_sequences=True)    #返回隐藏状态序列
        self.backward_rnn = SimpleRNN(128, return_sequences=True, go_backwards=True) #从后向前处理序列，返回逆向处理的隐藏状态序列
        self.reverse = Lambda(lambda x: tf.reverse(x, axis=[1]))  #三个维度，在维度1上对数据进行逆序
        self.concatenate = Concatenate(axis=2) #在轴2上拼接张量
        self.conv = Conv1D(64, kernel_size=1, activation='tanh') #64是卷积中滤波器的输出数量
